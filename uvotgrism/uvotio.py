...
def write_response_arf (wheelpos,spectralorder,wave,specresp,specresp_err=None,
       anker=None,dxy_anker=None,fileversion='999',clobber=False):
   ''' create an ARF file 
   ...
   NW = len(wave)
   ax = np.arange(NW)
   dispcoef = np.polyfit(ax,wave,4)
   wavmin = np.polyval(dispcoef,ax-0.5)
   wavmax = np.polyval(dispcoef,ax+0.5)
   binwidth = wavmax-wavmin
   midwave = wave
   ...
   ax = list(ax)
   ax.reverse()
   col11 = fits.Column(name='ENERG_LO',format='E',array=energy_lo[ax],unit='KeV')
   col12 = fits.Column(name='ENERG_HI',format='E',array=energy_hi[ax],unit='KeV')
   col13 = fits.Column(name='WAVE_MIN',format='E',array=wavmin[ax],unit='angstrom')
   col14 = fits.Column(name='WAVE_MAX',format='E',array=wavmax[ax],unit='angstrom')
   col15 = fits.Column(name='SPECRESP',format='E',array=specresp[ax],unit='cm**2' )
   if specresp_err == None:
      cols1 = fits.ColDefs([col11,col12,col13,col14,col15])
   else:
      col16 = fits.Column(name='SPRS_ERR',format='E',array=specresp_err[ax],unit='cm**2' )
   ...
   tbhdu1.header.update('CREATOR','uvotio.py','MSSL uvotpy python library')
   ...
   if (anker != None) & (dxy_anker != None):
      ...
      tbhdu1.header.update('CBD80001','ANCHOR_RANGE("['+str(dxy_anker[0])+','+str(dxy_anker[1])+']")','calibrared range around anchor')  
...
def readFluxCalFile(wheelpos,anchor=None,option="default",spectralorder=1,chatter=0):
   ...
   option == 'default' and anchor position given (in detector 
      pix coordinates) returns the fits HDU and an 
      interpolating function fnorm(wave in A)for the flux 
      correction
...
def writeSpectrum(ra,dec,filestub,ext, Y, fileoutstub=None, 
  ...
  **Output file composition**
       ...
       Main header: RA DEC  extracted source; 
       ...
       and coincidence-loss factor as applied to the flux listed. 
  ...
  **history**
  ...
  '''	    
   ...
   #C1inv = uvotgetspec.polyinverse(C_1, dis)
   dis1 = uvotgetspec.pix_from_wave(C_1,wave, spectralorder=1)
   #dis1 = np.polyval(C1inv, wave) 
   binwidth = np.polyval(C_1,dis1+0.5) - np.polyval(C_1,dis1-0.5)      # width of each bin in A (= scale A/pix)
      
   sprate = (counts-bkg)*aper1corr/expospec1                            # net count rate, aperture corrected
   sp1rate_err = np.sqrt( np.abs((counts-bkg)*aper1corr+2*bkg) )/expospec1  # neglect aperture-correction-error, coi-error
   bgrate = bkg/expospec1                                               # background counts at spectrum
   
   #print "*** uvotio.writeSpectrum uvotgetspec.coi_func call"
   fcoi = uvotgetspec.coi_func(dis,wave,sprate,bgrate,sig1coef=sig1coef,option=2,\
     fudgespec=1.0,coi_length=26,frametime=hdr['framtime'], background=False, \
     sigma1_limits=[2.6,4.0], trackwidth = trackwidth,debug=False,chatter=1)
   bgcoi = uvotgetspec.coi_func(dis,wave,sprate,bgrate,sig1coef=sig1coef,option=2,\
     fudgespec=1.0,coi_length=26,frametime=hdr['framtime'], background=True, \
     sigma1_limits=[2.6,4.0], trackwidth = trackwidth, debug=False,chatter=1)
     
   bg1rate = (bg1[q1[0]][qwave]*2*trackwidth*np.polyval(sig1coef,x[q1[0]])[qwave]/expospec1) # estimate bg1 * width 
   #bg2rate = (bg2[q1[0]][qwave]*2*trackwidth*np.polyval(sig1coef,x[q1[0]])[qwave]/expospec1) # estimate
      
   # now do aperture correction to measured counts and derive background 
   # the error needs to be passed as a unique entity for counts and background (TBD) 
   # counts = counts * aper1corr #* coi_func(wave)
   # bkg    = bkg * aper1corr    #* bgcoi_func(wave)
   ...
   hnu = h_c_ang/(wave)
   # coi-corrected flux
   flux = hnu*sprate*fcoi(wave)*senscorr/specresp1func(wave)/binwidth   # [erg/s/cm2/angstrom]   #  coi correction
   flux_err = hnu*sp1rate_err*fcoi(wave)*senscorr/specresp1func(wave)/binwidth 
       
   # prepare for output   
   # extname_order
   spectrum_first = (channel, counts[rc], (np.sqrt(counts*aper1corr))[rc], quality[rc], aper1corr[rc], expospec1[rc],  )
   back_first = (channel, bkg[rc], (np.sqrt(bkg*aper1corr))[rc],quality[rc],aper1corr[rc], expospec1[rc])
   calspec_first = (dis,wave,sprate,bg1rate,bgrate,flux,flux_err,quality,aper1corr,expospec1,fcoi(wave),bgcoi(wave))
   ...
   if present2 & fit_second: 
	 ...
	 sp2rate_err = np.sqrt(np.abs(sp2counts+bg_2cnt)) *aper2corr / expospec2 # error in net count rate
	 bg_2rate = bg_2cnt  /expospec2                                 # scaled background with NO aperture correction

	 qual2 = qquality[q2[0]]                                        # quality at second order (shared with first order)
         #C2inv = uvotgetspec.polyinverse(C_2, (q2[0]-dist12) )
         #dis2 = np.array(np.polyval(C2inv, wave2) +0.5 ,dtype=int)  
	 dis2 = uvotgetspec.pix_from_wave( C_2, wave2,spectralorder=2 )
         binwidth2 = np.polyval(C_2,dis2+0.5) - np.polyval(C_2,dis2-0.5)
	 #sp2counts = sp2counts * aper2corr
	 #bg_2cnt    = bg_2cnt *aper2corr
	 pix2      = x[q2[0]]                                           # pixel number to align with first order
         ...
         fcoi_2 = uvotgetspec.coi_func(pix2,wave2,sp2rate,bg_2rate,sig1coef=sig2coef,option=2,\
            fudgespec=1.0,coi_length=26,frametime=hdr['framtime'], background=False, \
            sigma1_limits=[2.6,6.0], trackwidth = trackwidth,debug=False,chatter=1)
         ...
         flux2 = hnu2*sp2rate*fcoi_2(wave2)*senscorr/specresp2func(wave2)/binwidth2            # [erg/s/cm2/angstrom]
         flux2_err = hnu2*sp2rate_err*fcoi_2(wave2)*senscorr/specresp2func(wave2)/binwidth2 
      
         # collect data for output
         spectrum_second = (channel2, sp2counts[rc2], (np.sqrt(sp2counts))[rc2], qual2[rc2], aper2corr[rc2], expospec2[rc2] )
	 back_second = (channel2, bg_2cnt[rc2], (np.sqrt(bg_2cnt))[rc2], qual2[rc2], aper2corr[rc2],expospec2[rc2] )
         calspec_second = (pix2,wave2,sp2rate,bg_2rate,flux2,flux2_err,qual2,aper2corr, expospec2,fcoi_2(wave))
   else:
   ...
   if clobber == False:
      if os.access(outfile1,os.F_OK) ^ os.access(backfile1,os.F_OK) ^ os.access(outfile2,os.F_OK) ^ os.access(backfile2,os.F_OK):
         print 'Error: output file already present. '
         if write_rmffile & (not os.access(rmffile1,os.F_OK)):
             write_rmf_file (rmffile1, wave, hdr['wheelpos'],1, C_1, arf1=arf1, arf2=None, clobber=clobber,chatter=chatter  )
         if present2 & fit_second & write_rmffile & (not os.access(rmffile2,os.F_OK)):
            write_rmf_file (rmffile2, wave2, hdr['wheelpos'],2, C_2, arf1=None, arf2=arf2, clobber=clobber,chatter=chatter   )
	 if interactive:
   ...
   writeSpectrum_ (ra,dec,obsid,ext,hdr,anker,phx,phy,offset, ank_c, exposure,  
      history, spectrum_first, back_first, calspec_first, extimg,  outfile1, 
      backfile1, rmffile1, outfile2=outfile2, backfile2=backfile2, rmffile2=rmffile2, expmap=expmap,   
      spectrum_second = spectrum_second, back_second = back_second, calspec_second=calspec_second, 
      present2=(present2 & fit_second),clobber=clobber, chatter=chatter)
...
def writeSpectrum_ (ra,dec,obsid,ext,hdr,anker,phx,phy,offset, ank_c, exposure,  
   ...
   try:
      ...
      pass   
   tbhdu1.header.update('BINX',hdr['BINX'],'binning factor in X-axis')
   tbhdu1.header.update('BINY',hdr['BINY'],'binning factor in Y-axis')
   tbhdu1.header.update('WINDOWX0',hdr['WINDOWX0'],'lower left RAWX')
   tbhdu1.header.update('WINDOWY0',hdr['WINDOWY0'],'lower left RAWY')
   tbhdu1.header.update('WINDOWDX',hdr['WINDOWDX'],'RAW window width')
   tbhdu1.header.update('WINDOWDY',hdr['WINDOWDY'],'RAW window height')
   ...
   tbhdu1.header.update('TLMAX1',len(spectrum_first[0])-1,'Highest legal channel number')
...
#   tbhdu1.header.update('ANCRFILE','NONE  ','Ancillary response')
   ...
   tbhdu1.header.update('CRPIX1  ','(0,'+str(len(spectrum_first[0])-1)+')','Channel binning of the CHANNEL column')
   ...
   # 
   (dis,wave,sprate,bg1rate,bg2rate,flux,flux_err,quality,aper1corr,expospec1,coi_sp1,bgcoi_sp1)  = calspec_first
   ...
   col23 = fits.Column(name='BG_L   ',format='E',array=bg1rate,unit='c/s')
   col24 = fits.Column(name='BG_R   ',format='E',array=bg2rate,unit='c/s')
   ...
   if present2:  # second order
      (pix2,wave2,sp2rate,bg_2rate,flux2,flux2_err,qual2,aper2corr,expospec2,coi_sp2) = calspec_second
      ...
      cols2 = fits.ColDefs([col20,col21,col22,col23,col24,col25,col26,col27,col28,col29,col29A,col29B,\
          col30,col31,col32,col34,col35,col36,col37,col38A])
   else:   
      cols2 = fits.ColDefs([col20,col21,col22,col23,col24,col25,col26,col27,col28,col29,col29A,col29B])

   tbhdu2 = fits.new_table(cols2)
   tbhdu2.header.update('HISTORY','net spectrum and background')
   if history != None:
      msg1 = history.split('\n')
...
#   tbhdu2.header.update('EXTNAME','FIRST_ORDER_NET_SPECTRUM')
   ...
   try:
      ...
      pass   
   tbhdu2.header.update('BINX',hdr['BINX'])
   tbhdu2.header.update('BINY',hdr['BINY'])
   tbhdu2.header.update('WINDOWX0',hdr['WINDOWX0'])
   tbhdu2.header.update('WINDOWY0',hdr['WINDOWY0'])
   tbhdu2.header.update('WINDOWDX',hdr['WINDOWDX'])
   tbhdu2.header.update('WINDOWDY',hdr['WINDOWDY'])
   ...
   if present2:
     ...
     tbhdu1.header.update('CHANTYPE','PI','Type of channel PHA/PI')
     tbhdu1.header.update('TLMIN1  ',0,'Lowest legal channel number')
     tbhdu1.header.update('TLMAX1',len(channel2)-1,'Highest legal channel number')
     tbhdu1.header.update('POISSERR','F','Poissonian errors not applicable')
     tbhdu1.header.update('GROUPING',0,'No grouping of the data has been defined')
